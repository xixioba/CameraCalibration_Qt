...
class ChessBoard():
    ...
    def create(self):
        ...
        board = (imageSize[0]-suqrelen*self.partternSize[0],
                 imageSize[1]-suqrelen*self.partternSize[1])
...
class CharucoBoard():
    ...
    def create(self, numX, numY, dict: int, squareSize, imageSize: list, dpi: int):
        ...
        self.board = cv.aruco.CharucoBoard(
            (numX, numY), self.squareSize, self.markerSize, self.dictionary)
        minSize = min(imageSize[0]-squareSize*numX,
                      imageSize[1]-squareSize*numY)/2
        if minSize < 0:
            print('too much square!')
            return None
        print((int(imageSize[0]*dpi), int(imageSize[1]*dpi)), int(minSize*dpi))
        img = self.board.generateImage((int(imageSize[0]*self.dpi/2.54), int(
            imageSize[1]*self.dpi/2.54)), None, int(minSize*self.dpi/2.54), 1)
        return img

    def saveConfig(self, path):
        file = cv.FileStorage(
            path+'/charucoBoard_Parameter.yml', cv.FileStorage_WRITE)
    ...
    def loadFromFile(self, path):
        file = cv.FileStorage(
            path+'/charucoBoard_Parameter.yml', cv.FileStorage_READ)
        pass  # unimplemented
        file.release()
